...
Se comprobó también la descarga, verificación de huella y preparación inicial desde carpetas vacías ajenas al repositorio para los dos notebooks. El 7 de septiembre de 2026, **ambos notebooks se ejecutaron completos en Google Colab con CPU y Python 3.13.15**, después de reiniciar la sesión para cargar las dependencias instaladas. Las 23 celdas de código de vuelos y las 26 de tiroides terminaron sin errores, con sus gráficas y comprobaciones finales. Los enlaces internos al enunciado, la documentación y el otro notebook también se verificaron desde Colab.

La prueba detectó un conflicto entre el NumPy precargado por Colab y la versión instalada durante la sesión. El arranque ahora detiene el análisis con una instrucción de reinicio cuando hace falta, y contrasta versiones en disco y en memoria. Los resultados principales de Random Forest en vuelos y Gradient Boosting en tiroides se reprodujeron. KNN presentó diferencias pequeñas entre Windows y Colab (alrededor de 0,11 u.m. en MAE), sin cambiar el ranking ni el modelo seleccionado; se registran en la evidencia y no se modifican los parámetros para forzar coincidencia. Los tiempos dependen de la máquina: KNN y la validación cruzada de vuelos tardaron más en la CPU de Colab.

Evidencias: [entorno local](reportes/entorno_local.json), [validación independiente de regresión](reportes/regresion_validacion_independiente.json), [validación independiente de clasificación](reportes/clasificacion_validacion_independiente.json), [verificación de la reorganización y portabilidad](reportes/verificacion_reorganizacion.json), [ejecución de vuelos en Colab](reportes/regresion_verificacion_colab.json) y [ejecución de tiroides en Colab](reportes/clasificacion_verificacion_colab.json).
